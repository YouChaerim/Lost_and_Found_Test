...
export default function IdFindScreen() {
  ...
  const handleVerifyPress = () => {
    const user = mockData.users.find(user => user.phoneNumber === phoneNumber);
    if (user) {
      Alert.alert('인증 확인', `아이디는 ${user.username} 입니다.`, [
        { text: '확인', onPress: () => console.log('확인 버튼 클릭됨') },
      ]);
    } else {
      Alert.alert('인증 실패', '입력한 번호와 일치하는 사용자를 찾을 수 없습니다.', [
        { text: '확인', onPress: () => console.log('확인 버튼 클릭됨') },
      ]);
    }
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.tabContainer}>
        <TouchableOpacity style={styles.tab} onPress={() => navigation.navigate('IdFind')}>
          <Text style={styles.tabText}>아이디 찾기</Text>
          <View style={styles.underline} />
        </TouchableOpacity>
        <TouchableOpacity style={styles.tab} onPress={() => navigation.navigate('PasswordReset')}>
